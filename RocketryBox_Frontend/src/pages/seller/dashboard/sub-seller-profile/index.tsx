...
const SubSellerProfilePage = () => {
  ...
  const { permissions: _permissions } = usePermissions(); // Unused
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch('/api/v2/seller/team-auth/profile', {
        method: 'GET',
        headers: {
  ...
  const handlePasswordChange = async () => {
    try {
      ...
      const response = await fetch('/api/v2/seller/team-auth/change-password', {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${await secureStorage.getItem('auth_token')}`,
          'Content-Type': 'application/json'
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {profile.seller.phone && (
                <div className="flex items-center gap-2 text-gray-600">
                  <Phone className="w-4 h-4" />
                  <span className="text-sm">{profile.seller.phone}</span>
                </div>
              )}
            </CardContent>
          </Card>
